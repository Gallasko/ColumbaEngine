...
# Create test application from the in-repo template
create_test_app() {
    log_info "Creating test application..."

    local test_dir="$INSTALL_DIR/test-app"
    local template_dir="$INSTALL_DIR/ColumbaEngine/templates/quickstart"

    if [[ ! -d "$template_dir" ]]; then
        log_error "Project template not found: $template_dir"
        log_info "The checked-out engine version may predate the templates/ directory."
        log_info "Use --version main, or copy examples/EmptyAppTemplate manually."
        return 1
    fi
    ...
    log_success "Test application created in: $test_dir (from templates/quickstart)"
}
