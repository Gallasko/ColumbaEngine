...
TEST_F(VMTest, StackPushPop) {
    ...
    EXPECT_DOUBLE_EQ(popped.getDouble(), 42.0);
    assertStackEmpty();
}
...
TEST_F(VMTest, StackPeek) {
    ...
    EXPECT_DOUBLE_EQ(top.getDouble(), 3.0);
    assertStackSize(3);

    auto second = peekStack(1);
    EXPECT_DOUBLE_EQ(second.getDouble(), 2.0);
    assertStackSize(3);
}
...
TEST_F(VMTest, Division) {
    ...
    EXPECT_EQ(result, InterpretResult::OK);
    assertStackSize(1);
    assertStackTop(ElementType(10.0 / 3.0));
}
...
TEST_F(VMTest, Negation) {
    ...
    EXPECT_EQ(result, InterpretResult::OK);
    assertStackSize(1);
    assertStackTop(ElementType(-5.0));
}
...
TEST_F(VMTest, BooleanNot) {
    ...
    EXPECT_FALSE(top.isTrue());  // !true == false
}
