...
TEST_F(ConstantPropagationPassTest, BasicLocalConstantPropagation) {
    ...
    // Create: OP_Constant 5, OP_Set_Local 0, OP_Get_Local 0
    chunk.addConstant(ElementType(5), 1);   // Adds OP_Constant 0
    chunk.addCode(OpCode::OP_Set_Local, 1); // Set to local slot 0
    chunk.addCode(0, 1);                    // Local slot 0

    chunk.addCode(OpCode::OP_Get_Local, 1); // Load from local slot 0
    chunk.addCode(0, 1);                    // Local slot 0
...
TEST_F(ConstantPropagationPassTest, LocalConstantWithDifferentSlots) {
    ...
    // var y = 20 (slot 1)
    chunk.addConstant(ElementType(20), 1);
    chunk.addCode(OpCode::OP_Set_Local, 1);
    chunk.addCode(1, 1);  // slot 1
...
TEST_F(ConstantPropagationPassTest, LongConstantSupport) {
    ...
    // Manually add OP_LongConstant instruction
