...
namespace pg
{
    ...
    struct UICommand
    {
        ...
        {
            auto it = args.find(key);
            if (it == args.end() || it->second.isEmpty())
                return fallback;
        ...
        size_t argSize(const std::string& key, size_t fallback = 0) const
        {
            ...
            switch (v.type)
            {
                case ElementType::UnionType::SIZE_T: return v.get<size_t>();
                case ElementType::UnionType::INT:    return static_cast<size_t>(v.get<int>());
                default: return fallback;
            }
        }
        ...
        {
            auto it = args.find(key);
            if (it == args.end() || it->second.isEmpty() || it->second.type != ElementType::UnionType::STRING)
                return fallback;
        ...
        {
            auto it = args.find(key);
            if (it == args.end() || it->second.isEmpty() || it->second.type != ElementType::UnionType::BOOL)
                return fallback;
