...
bool attachStrokeRect2DObject(VM* vm, EntitySystem* ecs, Entity* entity, int argCount, Value* args)
{
    ...
    for (int i = 0; i < argCount; i += 2)
    {
        ...
        if (key == "strokeWidth")
            strokeWidth = detail::extractFloatArg(args, i + 1);
        else if (key == "gap")
            gap = detail::extractFloatArg(args, i + 1);
        else if (key == "doubled")
            doubled = detail::extractBoolArg(args, i + 1);
        else if (key == "cornerRadius")
            cornerRadius = detail::extractFloatArg(args, i + 1);
    }

    // Attach StrokeRect2DObject with parsed parameters
    auto comp = ecs->_attach<StrokeRect2DObject>(entity);

    comp->setColors(colors);
    comp->setStrokeWidth(strokeWidth);
    comp->setGap(gap);
    comp->setDoubled(doubled);
    comp->setCornerRadius(cornerRadius);

    LOG_INFO("ECS Serialization", "Attached StrokeRect2DObject to entity " << entity->id);

    return true;
}

// Register the StrokeRect2DObject component attach handler
REGISTER_COMPONENT_ATTACH_HANDLER(StrokeRect2DObject, attachStrokeRect2DObject);
...
struct StrokeRect2DObjectProxyMetadataRegistrar
{
    StrokeRect2DObjectProxyMetadataRegistrar()
    {
        ...
        metadata.properties.emplace("strokeWidth", PropertyMetadata{
            ...
            [](void* comp, VM* vm) -> Value {
                ...
                return makeFloatValue(c->getStrokeWidth());
            },
            [](void* comp, VM* vm, Value val) {
                auto* c = static_cast<StrokeRect2DObject*>(comp);
                (void)vm; // Suppress unused parameter warning
            ...
            [](void* comp) -> std::string {
                auto* c = static_cast<StrokeRect2DObject*>(comp);
                return std::to_string(c->getStrokeWidth());
            },
            [](void* comp, const std::string& val) {
                auto* c = static_cast<StrokeRect2DObject*>(comp);
                c->setStrokeWidth(std::stof(val));
            }
        });
        ...
        metadata.properties.emplace("gap", PropertyMetadata{
            ...
            [](void* comp, VM* vm) -> Value {
                auto* c = static_cast<StrokeRect2DObject*>(comp);
                (void)vm; // Suppress unused parameter warning
                return makeFloatValue(c->getGap());
            },
            [](void* comp, VM* vm, Value val) {
                auto* c = static_cast<StrokeRect2DObject*>(comp);
                (void)vm; // Suppress unused parameter warning
            ...
            [](void* comp) -> std::string {
                auto* c = static_cast<StrokeRect2DObject*>(comp);
                return std::to_string(c->getGap());
            },
            [](void* comp, const std::string& val) {
                auto* c = static_cast<StrokeRect2DObject*>(comp);
                c->setGap(std::stof(val));
            }
        });
        ...
        metadata.properties.emplace("doubled", PropertyMetadata{
            ...
            [](void* comp, VM* vm) -> Value {
                auto* c = static_cast<StrokeRect2DObject*>(comp);
                (void)vm; // Suppress unused parameter warning
                return makeBoolValue(c->getDoubled());
            },
            [](void* comp, VM* vm, Value val) {
                auto* c = static_cast<StrokeRect2DObject*>(comp);
                (void)vm; // Suppress unused parameter warning
                c->setDoubled(AS_BOOL(val));
            },
            [](void* comp) -> std::string {
                auto* c = static_cast<StrokeRect2DObject*>(comp);
                return std::to_string(c->getDoubled());
            },
            [](void* comp, const std::string& val) {
                auto* c = static_cast<StrokeRect2DObject*>(comp);
                c->setDoubled(val == "true" or val == "1");
            }
        });
        ...
        LOG_INFO("ComponentProxy", "Registered proxy metadata for StrokeRect2DObject");
    }
};

static StrokeRect2DObjectProxyMetadataRegistrar s_strokeRect2DObjectProxyMetadataRegistrar;
