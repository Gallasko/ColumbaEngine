...
layout (location = 7) in float aGap;
layout (location = 8) in float aDoubled;
...
out vec2  fragUV;
out vec2  fragSize;
flat out float fragStrokeWidth;
flat out float fragGap;
flat out float fragDoubled;
...
void main()
{
    ...
    fragUV          = aTexCoord;
    fragSize        = aSize;
    fragStrokeWidth = aStrokeWidth;
    fragGap         = aGap;
    fragDoubled     = aDoubled;
    fragRadius      = aRadius;
}
